...
1. `{{EVENTS_JSONL_PATH}}` -- the raw event stream. Extract:
   ...
   - Compaction and resume boundaries are not in the events log: read the raw transcript. Each compaction writes one `system` record whose `subtype` is `compact_boundary`; count compactions from those records. When SessionStart hooks are installed, each resume writes one `attachment` record per hook whose `attachment.hookName` is `SessionStart:resume` (a compaction writes matching `SessionStart:compact` records); count each run of consecutive `SessionStart:resume` records (adjacent lines; their timestamps can differ by a millisecond) as one resume. If no `attachment.hookName` in the transcript starts with `SessionStart`, no hook was installed: report resume ordering as not measured.
