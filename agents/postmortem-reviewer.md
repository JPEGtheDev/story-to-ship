...
| Correction-source audit: list every dispatcher statement of fact in an `assistant.message` that a later event contradicts or retracts. Classify each: SELF-CAUGHT when the retraction appears in an `assistant.message` (the same message counts) with no intervening `user.message` or `subagent.completed` that raises it; EXTERNALLY-CAUGHT otherwise. Report the two counts and the ratio externally-caught / (self-caught + externally-caught) here; itemize every item in Step 3 under "Correction-source itemization". A row answer without the Step 3 list is an incomplete report. Ignore `assistant.message` events whose model is the literal `<synthetic>`. | |
