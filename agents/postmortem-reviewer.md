...
1. `{{EVENTS_JSONL_PATH}}` -- the raw event stream. Extract:
   ...
   - Compaction and resume boundaries are not in the events log: read the raw transcript. Each compaction writes one `system` record whose `subtype` is `compact_boundary`; count compactions from those records. When SessionStart hooks are installed, each resume writes one `attachment` record per hook whose `attachment.hookName` is `SessionStart:resume` (a compaction writes matching `SessionStart:compact` records); count records that share one timestamp as one resume. If no `attachment.hookName` in the transcript starts with `SessionStart`, no hook was installed: report resume ordering as not measured.
...
| Continuation-reload audit: after each compaction or resume boundary (the boundary records Step 1 names), was the agent's next `skill.invoked` a re-invocation of `session-bootstrap` (and `honesty` and `communication`) BEFORE any `Edit`/`Write`/`NotebookEdit`/`Bash` call or `subagent.started` -- or did it act first and reload later (or not at all)? On any disagreement between the events log and the raw transcript about first-action ordering, tools/first_action_audit/first-action-audit.sh run against {{TRANSCRIPT_PATH}} is the authoritative tiebreaker -- quote its VERDICT line. Then, for each boundary, list every skill invoked before it that was not loaded solely for a finished todo (when the log does not show which todo a skill served, count it in), and name each one not re-invoked before the next call that writes a repo file (`Edit`, `Write`, `NotebookEdit`, or a `Bash` command that writes a repo path; the plan file counts) or `subagent.started` -- the `session-bootstrap` skill's dispatch-or-edit window, so unlike the ordering check above a `Bash` call that writes no repo path does not close the window; a skill missing from the reload set is a finding even when the first action after the boundary was clean. | |
