...
Unlike `workflow-model-guard.sh`, there is no marker, env var, or config option that lifts the guard for one command. The sanctioned route to overwrite a protected file is Read, then Edit or Write. Subagents are not exempt from the overwrite rule: it ignores an `agent_id` field on the hook payload, deliberately differing from the three sibling guards, because a subagent's shell overwrite is the same defect as one from the main session.

Main-session rule: for a call with no `agent_id` (the main session, not a dispatched subagent), the guard also denies creating a new file and appending to a file (`>>`, `tee -a` or `--append`, an interpreter `open()` in `a` or `x` mode, `writeFileSync` to a new path) on any repository path that is not gitignored. The main session's repository changes then go through the Edit and Write tools, where `inline-edit-guard.sh` counts them. A dispatched subagent keeps the earlier behaviour: new files and appends are allowed for it. Paths outside any repository and gitignored paths are unaffected for every caller.
