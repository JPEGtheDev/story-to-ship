...
# a missing loaded file -- falls back to the full text. Some cases go the
# other way (see Documented limitation in README.md in this directory); one
# is a state dir that exists but cannot be written: it gets no flags from
# session-start.sh, so this hook emits the loaded text from the first turn.
