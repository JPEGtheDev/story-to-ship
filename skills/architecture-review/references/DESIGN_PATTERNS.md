...
## Framework Edge Wrapper
Polished framework widgets and drag-and-drop controls make the common 80% of a task effortless, but push back hard once you step outside that path--a dropdown that offers no way to group its options, or a connection string the built-in control will not let you change once the program is running. Rather than fighting the built-in abstraction further to cover that gap, write a small, purpose-built wrapper scoped to just the failing edge--a thin HTML-generation helper or a hand-rolled connection wrapper are typical shapes--and confirm it behaves correctly by inspecting the output it actually produces. Unlike a Facade, which simplifies an entire subsystem, this wrapper covers only the one case the framework does not, and unlike a general abstraction layer, it is justified by a demonstrated failure rather than anticipated need. The trade is deliberate, not free: the wrapper gives up the framework's IDE (Integrated Development Environment) design-time visualization, autocomplete-style assistance, and some compile-time structural checks in exchange for readability, portability, and runtime flexibility. (Source: C2 Wiki "DotNet")
...
## No Abstraction Without Two Cases (YAGNI (You Ain't Gonna Need It) for Generality)
...
- the `oop-principles` skill -- Is-A / Has-A hierarchy design
