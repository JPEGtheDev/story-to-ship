...
Illustration (generic, not from the source): two engineers both say "caching will break consistency" and appear to be arguing -- one is describing a write-through application cache, the other a CDN edge cache. Both statements are true of their own referent; once the two caches are named separately, there is no actual disagreement left to have.
