...
> Solution: `mkdir -p docs/` (and `docs/dod/<group-slug>/` per the template's group-slug transform, as needed), then write in exactly this order: detail files (only for layers whose elaboration needs one) -> `docs/DOD.md` -> `docs/INDEX.md` row for DOD.md. Formats per `references/DOD_TEMPLATE.md` exactly: cross-cutting frontmatter, ruling lines, `Stamp: vN` from the taxonomy's current stamp, and a `Content-hash:` line. Compute the hash over exactly the byte range `references/DOD_TEMPLATE.md` Section A.4 defines (every byte through the end of the line preceding the `Content-hash:` line): write the file's content without that line, hash those bytes with SHA-256, then append `Content-hash: sha256:<64-hex>` as the file's final line. This same computation applies at every canon write, fresh or delta.
