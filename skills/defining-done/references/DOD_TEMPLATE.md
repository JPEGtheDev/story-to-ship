...
- `DOD-GATE: FAIL <layer>` -- the completion gate's failure marker, emitted when a
  ...
  trigger fired. Consumers are also permitted, but not required, to ground cited
  evidence against the repo under evaluation; when that grounding shows the cited
  evidence does not hold there, the same marker MAY be emitted -- a grounding-driven
  failure is a legitimate gate outcome, not a defect in the consumer or the
