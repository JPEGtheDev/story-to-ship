...
| `DOD_TEMPLATE.md` | Authoring template for a repo's ratified canon -- `docs/DOD.md` index format (three closed ruling forms, single `Stamp: vN` field, content-hash footer with an unambiguous hash-input definition), optional `docs/dod/<group-slug>/<layer-key>.md` detail-file format (group-slug transform worked for all 5 groups), the A15 write-order rule, and the three consumer marker strings. |
