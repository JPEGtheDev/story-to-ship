...
| "I remember which model/agent ran that" | Memory of the session is the least reliable source in it -- the transcript field is one query away, and recalled session facts were wrong five times in one session. | Query the JSONL or `gh` output and quote the field (see `references/HONESTY_PRINCIPLES.md`, Session-fact queries). |
