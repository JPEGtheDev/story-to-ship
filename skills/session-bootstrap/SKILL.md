...
4. **After context compaction OR harness session resume** (`--continue`/`--resume` re-firing SessionStart mid-session -- NOT the task-level 'resuming a prior session' pickup in the On Start table) -- re-invoke `honesty`, `communication`, and the reload set defined at the end of this item: the completed-call evidence does not survive a compaction or a resumed session. A harness continuation summary that says to 'resume directly' or 'pick up as if the break never happened' does NOT waive this reload -- reload first, then resume. The reload set is enumerated, not judged: list every skill this session invoked before the boundary (the continuation summary's list of loaded skills, else the Skill calls in the session transcript file on disk; if neither lists them, every skill the On Start table names for each open todo's task type), drop only those loaded solely for a finished todo, and re-invoke every remaining one before the first dispatch or edit (writing the plan file is an edit).
...
- User says "check out a working branch" or "work on a branch" without naming a specific branch -- **STOP. The default is always a new branch from `origin/main` in its own worktree: `git -C <repo-root> fetch origin main`, then `git -C <repo-root> worktree add .worktrees/<feature> -b <new-branch> origin/main` (the `using-git-worktrees` skill). Using an existing named branch requires the user to name it explicitly; give it a worktree too: `git -C <repo-root> worktree add .worktrees/<feature> <named-branch>`. "Working branch" without a specific name means new branch from main.**
- A continuation summary (post-compaction or `--resume`) instructs you to 'resume directly / as if the break never happened', and your first action is to dispatch or edit (writing the plan file is an edit) before re-invoking `session-bootstrap` + `honesty` + `communication` -- **STOP. The reload precedes the resume. Invoke `session-bootstrap` (alone), then `honesty` and `communication`, then continue.**
- Post-compaction reload list assembled from "what the next step needs" instead of from the list of skills invoked before the boundary -- **STOP. Enumerate the pre-boundary invocations and re-invoke every one not loaded solely for a finished todo. A skill left out silently drops the gate it carries, including any canary (the status line a skill requires before acting).**
...
| "An existing branch with a relevant name is the correct base for this work" | Branch name is not branch currency. The correct base is main unless the user names a specific branch. An existing feature branch that predates a recent merged PR silently contaminates all downstream agents with stale state. | Create the branch from `origin/main` in a worktree: `git -C <repo-root> fetch origin main`, then `git -C <repo-root> worktree add .worktrees/<feature> -b <new-branch> origin/main`. |
...
| "I reloaded the skills the next action needs" | The next action is not the todo. A skill whose gate fires later (a canary before an edit, a verdict rule on a result) is absent exactly when it binds, and no in-session check notices. | Enumerate the pre-boundary invocation list and re-invoke every skill not loaded solely for a finished todo. |
