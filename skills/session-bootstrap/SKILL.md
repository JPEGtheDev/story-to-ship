...
- Post-compaction reload list assembled from "what the next step needs" instead of from the list of skills invoked before the boundary -- **STOP. Enumerate the pre-boundary invocations and re-invoke every skill whose todo is still open. A skill left out silently drops the canary or gate it carries.**
