...
3. Read `checkpoints/index.md` to determine the full session scope. Context compaction does not shorten the session. A self-assessment that covers only the final task of a 9-hour session is incomplete. If `checkpoints/` does not exist, note the absence and determine scope from the compacted context summary and `events.jsonl` instead.
4. Self-evaluation skill run and output written to `self-assessment.md` in the repo root. The external reviewer reads `self-assessment.md` to find discrepancies between what the agent claimed and what the log shows. A postmortem without a self-assessment is missing the agent's own perspective.
...
[-] Any unmet -> STOP. Wait for the session to complete, locate the events log, read checkpoints/index.md (or note its absence), run self-evaluation and write self-assessment.md, then dispatch the external reviewer.
