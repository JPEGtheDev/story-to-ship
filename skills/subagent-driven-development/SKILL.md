...
**Re-review required for review-covered territory:** The GAPS/REQUEST CHANGES re-run rules above are the specific case of a general rule: any change that lands in territory an already-completed review covered -- a post-review edit, a fix round touching reviewed lines, a "small" amendment to an approved diff -- requires re-review before the work advances. The prior review's PASS/APPROVE does not extend to the new change, including changes the agent itself initiates after that verdict. The sole exemption is an explicit user waiver given in the same turn as the change -- not an inferred waiver, not a prior-turn "go ahead", not the agent's own judgment that the change is trivial or already covered by other verification. Enforcement is procedural, not mechanical: a post-review commit touching review-covered territory with no re-review dispatch visible in the transcript is the checkable signal of a violation; no automated detector exists for this.
...
| "I've already verified this change through mutation testing, which is more rigorous than a re-review would be -- I'll proceed with committing" | This is the exact shape of a documented failure in the source postmortem: an agent self-waiving a mandated Stage 2 re-review after a REQUEST CHANGES fix on the grounds that its own verification was "more rigorous." The agent adjudicating whether its own change is "covered enough" to skip re-review is the failure mode, not a valid exemption. YOU MUST re-review any change in review-covered territory; the sole exemption is an explicit user waiver given in the same turn. |
