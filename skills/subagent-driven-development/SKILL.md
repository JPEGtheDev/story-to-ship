...
9. Every verification command written into the prompt file -- an implementer's or a reviewer's (a grep, wc, sed, or test the agent must run and paste) -- was run by the dispatcher against one input where it must pass and one where it must fail, with both outputs pasted in the dispatch turn (the reply that sends the dispatch). For a check on text not yet written, the must-pass input is a scratch copy of the target file with the intended edit applied; the must-fail input is the unedited file for a check that asserts the change, and a copy with the kept text altered for a check that asserts something the edit must keep (a count, a size, a required string). A check that passes on both, or that cannot be made to fail, is redesigned before dispatch. Enforcement is procedural self-check; the after-the-fact detector is the postmortem-reviewer template's Dispatch-check audit row, and a verification command with no paired pass/fail run above it in the dispatch turn is the violation.
...
| "Dispatching any agent without passing `model`, or general-purpose without naming the template considered" | STOP. An omitted `model` falls back to the template pin or inherits your model, and the events log labels both `inherit`. Pass `model` explicitly on every call; for general-purpose also name the template that does not fit and the tier reasoning (`references/MODEL_SELECTION.md`). |
