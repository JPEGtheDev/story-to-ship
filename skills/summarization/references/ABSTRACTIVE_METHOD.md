...
3. **Compress without omitting key concepts.** The output must be materially shorter than the source while retaining every idea that matters.
...
Output length is a soft target. Prioritize retaining key concepts over brevity.
