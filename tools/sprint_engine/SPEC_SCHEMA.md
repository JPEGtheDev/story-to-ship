...
| `model` (on an agent or gate step) | OPTIONAL | Selects which model the runner dispatches that step's agent call to. Pass-through only: the engine does not validate the value against any known-model list, and neither does the runner glue -- an unrecognized value is a harness-level failure at dispatch time, not a spec-validation error. When present, this step's own `model` overrides `config.model` for that one step. |
